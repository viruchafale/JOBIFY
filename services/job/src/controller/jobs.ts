...
export const deleteCompany = TryCatch(
  ...
  `;
    ...
    if (!company) {
      throw new ErrorHandler(
        404,
        "Company not found oro you're not authorized to delete it.",
      );
    }
...
export const getSingleJobs = TryCatch(async (req, res) => {
...
export const getAllApplicationForJOb = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (user.role !== "recruiter") {
      throw new ErrorHandler(
        403,
        "Forbidden:Only recruiter can access applications ",
      );
    }
...
export const updateApplication=TryCatch(async (req:AuthenticatedRequest,res)=>{
  const user = req.user;
    ...
    const message={
      ...
      subject:"Application updates -Job portal",
